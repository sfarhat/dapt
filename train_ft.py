# Fully finetuning networks on downstream tasks
...
def main(args):
    ...
    if 'resnet' in args.model:
        ...
        params_1x = [param for name, param in net.named_parameters() if 'heads' not in str(name)]
    else:
        raise NotImplementedError(args.model)

    if args.optimizer == 'adamw':
        # default adam lr is 1e-3, so we set head to that
        adam_lr  = 1e-4
        optimizer = torch.optim.AdamW([{'params':params_1x}, {'params': net.get_classifier().parameters(), 'lr': adam_lr*10}], lr=adam_lr, weight_decay=args.weight_decay)
    elif args.optimizer == 'sgd':
        optimizer = torch.optim.SGD([{'params':params_1x}, {'params': net.get_classifier().parameters(), 'lr': args.learning_rate*10}], lr=args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    dir_name = f"{args.model}_ft_{args.optimizer}"
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Finetune Dataset')
